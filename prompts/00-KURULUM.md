...
Bu dosya mekanik bir kurulum işidir, güçlü modele gerek yok.
...
1. **Kullanıcıya soru sorma.** Bu paketi çalıştıran kişi teknik değil ve sorulara cevap
   veremez. Belirsizlik varsa bu dosyalardaki kuralı uygula; kural yoksa en muhafazakâr
   seçeneği seç, işe devam et ve kararını `NOTLAR.md` dosyasına yaz.
2. **İşi yarım bırakma.** Hedef sayıya ulaşana kadar üret.
3. **Sonunda mutlaka commit + push yap.** Adımlar bu dosyanın sonunda.
4. **Telif:** aşağıdaki "Telif kuralları" bölümü mutlak. İhlali projeyi bitirir.
...
Aşağıdaki **"SORU DAĞILIM PLANI"** bölümünün tamamını, başlığından itibaren birebir
kopyalayıp `content/PLAN-soru-dagilimi.md` dosyası olarak kaydet. Sonraki bütün promptlar
bu dosyayı okuyacak. **Hiçbir şey değiştirme, kısaltma, yorumlama — birebir kopyala.**

## Adım 5 — NOTLAR.md oluştur

`NOTLAR.md` adında bir dosya oluştur, içine şunu yaz:
...
Push hata verirse `git pull --rebase` komutunu tekrar çalıştırıp tekrar `git push` dene.

Bittiğinde kullanıcıya tek cümleyle "Kurulum tamam, sıradaki dosya: `01-pasaj-secimi.md`" de.
