"""OPUS5-21 / 11. paket denetimi: content/listening/practice/short-answer.json

Kullanim:  python tools/_p11_kontrol.py [hedef.json]

Hicbir dosyayi degistirmez. Uretilen alistirma setini diskten geri okuyup
promptun teslim listesindeki her maddeyi tek tek sinar. 9. paketin
denetiminden iki farki var:

1. Kisa cevapta bosluk isareti yok; `prompt` bir sorudur. O yuzden
   "(n) ........" araniz yerine soru isareti aranir.
2. Cevap tek basina anlamli olmali: soru koku sesteki cumleyi tekrar
   etmemeli (2. altin kural) -- bu kontrol sadelestirilmis metin
   karsilastirmasiyla yapilir, kok ile replik cumlesi karsilastirilir.

Argumanla baska bir alistirma dosyasi verilirse (orn. 10. paketin akis
semasi seti) ayni kontroller o dosyaya uygulanir; bosluk isareti kontrolu
kisa cevap disindaki tiplerde "(n) ........" seklinde calisir.
"""
...
HEDEF = sys.argv[1] if len(sys.argv) > 1 else "content/listening/practice/short-answer.json"
BEKLENEN_SORU = 15
...
def main():
    ...
    for g in d["groups"]:
        ...
        if kisa_cevap and (g.get("stem_block") is not None or g.get("table") is not None):
            hata.append("%s: kisa cevapta stem_block ve table null olmali" % gid)
        ...
        for it in g["items"]:
            ...
            # soru koku
            if kisa_cevap:
                if not it["prompt"].rstrip().endswith("?"):
                    hata.append("%s: kisa cevap sorusu soru isaretiyle bitmiyor" % etiket)
                if "........" in it["prompt"]:
                    hata.append("%s: kisa cevapta bosluk isareti olmamali" % etiket)
                kok = sadelestir(it["prompt"])
            else:
