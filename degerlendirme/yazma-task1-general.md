...
3. Read the criterion's table **from the top down**, starting at band 9. Step down one row at a time
   and stop at the first row that is a true description of this letter: the band is the **highest
   row that is still true**, not the row that feels safest. Never start in the middle of the table
   and work outwards. If the letter genuinely sits between the row you stopped at and the one above
   ...
   for a borderline letter. This stopping test is complete only for rows of 7 and above; for rows of
   6 and below rule 5 tells you how far to keep going.
4. Bands 7, 8 and 9 describe letters that **still contain faults** — read the rows: 7 allows
...
5. Bands 6 and below describe what a letter **fails** to do, and those descriptions stack: a letter
   that matches band 4 also makes most of the band 5 row read as true, and a band 3 letter makes
   ...
   below, read the row **beneath** it as well and ask which of the two describes this letter
   better; keep stepping down while the lower row is the better description, and stop only when it
   is plainly too severe. Award the row that describes the letter, not the highest row it does not
   contradict.
6. At the bottom of the scale the question is never "can I find something the letter does?" (you
   ...
   paragraphing, one bullet answered is not coverage, a phrase that can be reconstructed out of a
   garbled sentence is not communication. Something the letter manages is a reason to award 4
...
7. Apply the caps listed under each criterion, **last of all**. A cap is a **ceiling, never a
   ...
   its condition is plainly true of this letter and you can point at the evidence; if you have to
   argue it into place, it does not fire. The number of caps that fire is not evidence of anything:
   if several fire, or the letter also matches a descriptor row below the cap, the band is the one
   you read off the table. A band equal to a cap value has to be **earned twice**: the cap has to
   allow it *and* the table row has to describe the letter on its own. A cap fires because
   ...
   after all. If two or more caps fire on the same criterion the letter is weak in several ways at
...
**6 against 7:** a bullet is covered when the reader is given content they could act on. One
well-chosen sentence can cover a bullet; judge the content delivered, not the number of lines spent
on it or whether the bullet got its own paragraph. A bullet that is covered but could have been
illustrated more fully is the "one is thin" of band 7, not the "barely touched" of band 5.

**5 against 4:** at 5 the letter does its job badly — the purpose wavers, one bullet is thin, the
tone is uneven. At 4 the letter does not do its job: the reader would be left without information
they actually need. At 3 the reader is left with almost nothing they could act on — and a letter
can reach 3 with all three bullets mentioned, if what is said about them cannot be used. A letter
that trips a "max 5" cap has not thereby earned 5; read the 4 and 3 rows too before settling.
...
**5 against 4 against 3:** at 5 the reader can follow the letter but does some of the work
themselves. At 4 the reader has to rebuild the order — the points arrive in whatever sequence they
occurred to the writer. At 3 the relations between the points cannot be recovered at all. Breaks on
the page are not paragraphing unless each block holds one idea, and points arriving in a sensible
order do not reach 6 if the sentence boundaries inside them have collapsed. Judge how much work the
...
- Written as one undivided block with no paragraphing → **max 6**. Missing paragraphing keeps a
  letter out of 7 and above; it does not by itself place it at 5. Drop to **max 5** only when the
  missing breaks are what makes the order of the points hard to follow — if the sequence is clear
...
- Sentence boundaries break down (missing full stops, run-ons) badly enough that the reader has to
  ...
  through the whole letter and the thread is genuinely lost. Count the sentences you actually had
...
**5 against 4 against 3:** at 5 the reader understands the letter and notices the strain. At 4 the
...
of leeway below never lifts a letter across a row boundary on its own.
...
- The **first sentence** of `why` says what the letter actually does that earns the band you
  ...
  is usually a band too low: the easiest thing to name in any letter is an error, so a rule that
...
- Naming that strength is a requirement of the **format**, not evidence for the band. At the bottom
  of the scale it is usually the only strength there is, and it does not lift the letter out of the
  row it belongs to. Write the sentence, then award the band the letter as a whole earns — if the
